...
from .ble import StudioBLE, scan_for_studio_devices
...
async def discover_ble() -> StudioTransport:
    ...
    LOG.info("Scanning for BLE ZMK Studio devices...")
    devices = await scan_for_studio_devices()
    if not devices:
        raise DeviceNotFoundError(
            "No BLE devices advertising the ZMK Studio service UUID were found. "
            "Make sure the Toucan LH is powered on and in range."
        )
    last_err: Exception | None = None
    for device in devices:
        LOG.debug("Probing BLE device %s (%s)", device.address, device.name)
        ...
        try:
            ...
            continue
    raise DeviceNotFoundError(
        f"Found BLE devices but none responded to ZMK Studio RPC. "
        f"Last error: {last_err}"
    )
